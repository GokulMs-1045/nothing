...
export const createOrderbyId = async (req, res) => {
  try {
    ...
    const {
      ...
      deliveryMode,
      paymentMode,
      paymentFile, // Extract paymentFile from body
      shippingDetails,
    } = req.body;
    ...
    if (!deliveryTime || !['12 AM', '1 AM', '2 AM', '3 AM', '4 AM', '5 AM', '6 AM', '7 AM', '8 AM', '9 AM', '10 AM', '11 AM', '12 PM', '1 PM', '2 PM', '3 PM', '4 PM', '5 PM', '6 PM', '7 PM', '8 PM', '9 PM', '10 PM', '11 PM'].includes(deliveryTime)) return res.status(400).json({ message: 'Invalid delivery time' });
    ...
    if (paymentMode === 'Online Payment' && (!paymentFile || typeof paymentFile !== 'string' || paymentFile.trim() === '')) {
      return res.status(400).json({ message: 'Payment file (string) is required for online payment' });
    }
    ...
    const orderData = {
      ...
      deliveryMode,
      paymentMode,
      paymentFile: paymentMode === 'Online Payment' ? paymentFile : null, // Only include if Online Payment
      shippingDetails,
    };
...
export const getOrdersById = async (req, res) => {
  try {
    ...
    if (productName) {
      ...
      query.productName = product._Name;
    }
...
export const updateOrderById = async (req, res) => {
  try {
    ...
    if (productName) {
      ...
      updateData.productImage = product.productImage;
